...
- Any `Mapping` is accepted, not only `dict`. A `MappingProxyType`, a multidict,
  ...
  voluptuous, so a Home Assistant `NodeDictClass` keeps its type across
  validation, and the source file and line it carries survive too once the class
  opts in to [annotations](/guides/annotations/). A strict superset, so dict code
  is unaffected.
